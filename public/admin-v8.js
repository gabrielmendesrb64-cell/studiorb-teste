...
async function refresh(){
  ...
  renderStats(); renderBlocks(); renderCalendar(); renderList(); renderSchedule(); renderServices(); renderNotificationStatus(n);
}
...
function renderList(){
  ...
  $('#bookingList').innerHTML=list.length?list.map(b=>{
    ...
    const confirmMsg=`Olá, ${b.name}! 💗 Seu agendamento no LSH Studio RB foi confirmado para ${fmtDate(b.date)} às ${b.time}. Procedimento(s): ${services}. Total: ${money(b.total)}.`;
