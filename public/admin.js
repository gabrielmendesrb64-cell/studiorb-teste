...
function actionButton(text, cls, handler) { const b = document.createElement('button'); b.className = cls; b.type = 'button'; b.textContent = text; b.addEventListener('click', handler); return b; }
...
function giftItem(gift) {
  ...
  if (gift.image) {
    const img = document.createElement('img'); img.src = gift.image; img.alt = gift.name; img.referrerPolicy = 'no-referrer';
  ...
  const meta = document.createElement('div'); meta.className = 'admin-meta'; meta.textContent = gift.category || 'Outros';
  info.append(h, meta);
  if (gift.purchaseUrl) {
    const a = document.createElement('a'); a.className = 'admin-product-link'; a.href = gift.purchaseUrl; a.target = '_blank'; a.rel = 'noopener noreferrer nofollow'; a.textContent = 'Ver presente desejado ↗'; info.appendChild(a);
  }
  if (gift.reservedBy) {
    const who = document.createElement('div'); who.className = 'admin-meta reserved-who';
    who.textContent = `Por: ${gift.reservedBy.name}${gift.reservedBy.phone ? ` • ${phoneMask(gift.reservedBy.phone)}` : ''}${gift.reservedBy.reservedAt ? ` • ${dateText(gift.reservedBy.reservedAt)}` : ''}`;
...
function fillList(selector, items, emptyText) {
  const wrap = $(selector); wrap.replaceChildren();
  if (!items.length) { const e = document.createElement('div'); e.className = 'mini-empty'; e.textContent = emptyText; wrap.appendChild(e); return; }
  items.forEach(g => wrap.appendChild(giftItem(g)));
}

function renderGiftSections() {
  const gifts = dashboardData.gifts || [];
  const a = gifts.filter(g => g.status === 'available');
  const r = gifts.filter(g => g.status === 'reserved');
  const d = gifts.filter(g => g.status === 'received');
  $('#countAvailable').textContent = a.length; $('#countReserved').textContent = r.length; $('#countReceived').textContent = d.length;
  fillList('#availableGiftList', a, 'Nenhum presente disponível.');
  fillList('#reservedGiftList', r, 'Nenhum presente reservado.');
  fillList('#receivedGiftList', d, 'Nenhum presente entregue ainda.');
}

function rsvpItem(r) {
  const names = rsvpNames(r);
  const item = document.createElement('article'); item.className = 'admin-item rsvp-item';
  const info = document.createElement('div');
  const h = document.createElement('h4'); h.textContent = `${names.length} ${names.length === 1 ? 'pessoa confirmada' : 'pessoas confirmadas'}`;
  const meta = document.createElement('div'); meta.className = 'admin-meta'; meta.textContent = r.confirmedAt ? `Enviado em ${dateText(r.confirmedAt)}` : 'Confirmação registrada';
  const list = document.createElement('div'); list.className = 'rsvp-names';
  names.forEach(name => { const n = document.createElement('div'); n.className = 'rsvp-name'; n.textContent = `• ${name}`; list.appendChild(n); });
  info.append(h, meta, list);
  const actions = document.createElement('div'); actions.className = 'admin-actions';
  actions.appendChild(actionButton('Excluir', 'btn btn-danger btn-small', async () => { if (confirm('Excluir esta confirmação de presença?')) await mutate(`/api/admin/rsvps/${r.id}`, 'DELETE'); }));
...
function renderRsvps() {
  ...
  const wrap = $('#rsvpYesList'); wrap.replaceChildren();
  if (!items.length) { const e = document.createElement('div'); e.className = 'mini-empty'; e.textContent = 'Ninguém confirmou presença ainda.'; wrap.appendChild(e); return; }
  items.forEach(r => wrap.appendChild(rsvpItem(r)));
}

async function previewProduct() {
  const url = $('#giftProductUrl').value.trim(); const box = $('#productPreview');
  ...
  try {
    ...
    if (p.image) { $('#productPreviewImage').src = p.image; $('#giftImage').value = p.image; $('#productPreviewStatus').textContent = 'Foto encontrada automaticamente.'; }
    else $('#productPreviewStatus').textContent = 'A página foi encontrada, mas a loja não informou uma foto. Cole o link da imagem manualmente abaixo.';
...
$('#giftForm').addEventListener('submit', async e => {
  e.preventDefault(); $('#giftError').textContent = ''; $('#giftSuccess').classList.add('hidden');
  try {
    const result = await api('/api/admin/gifts', { method: 'POST', body: JSON.stringify({ name: $('#giftName').value, purchaseUrl: $('#giftProductUrl').value, image: $('#giftImage').value, category: $('#giftCategory').value, description: $('#giftDescription').value }) });
    e.currentTarget.reset(); $('#productPreview').classList.add('hidden'); $('#giftSuccess').textContent = result.warning || 'Presente adicionado com sucesso!'; $('#giftSuccess').classList.remove('hidden'); await loadDashboard();
  } catch (err) { $('#giftError').textContent = err.message; }
});

$('#configForm').addEventListener('submit', async e => {
  e.preventDefault(); $('#configError').textContent = ''; $('#configSuccess').classList.add('hidden');
  try {
    ...
    $('#configSuccess').textContent = 'Informações do chá e do PIX salvas.'; $('#configSuccess').classList.remove('hidden'); await loadDashboard();
